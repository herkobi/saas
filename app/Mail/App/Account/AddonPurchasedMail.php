...
/**
 * Addon Purchased Mail
 *
 * This mailable is sent when a tenant purchases an addon.
 *
...
/**
 * Mailable for addon purchase notifications.
 *
 * Sends an email with addon details and purchase information.
 */
class AddonPurchasedMail extends Mailable
{
    ...
    {
        return new Envelope(
            subject: 'Eklenti Satın Alma Onayı',
        );
    }
    ...
    public function content(): Content
    {
        ...
        return new Content(
            markdown: 'mail.tenant.account.addon-purchased',
            with: [
                'user' => $this->user,
                'addon' => $this->addon,
                ...
                'totalPrice' => number_format($this->tenantAddon->getTotalPrice(), 2),
                'currency' => $this->tenantAddon->getEffectiveCurrency(),
                'startedAt' => $this->tenantAddon->started_at?->format('d.m.Y H:i'),
                'expiresAt' => $this->tenantAddon->expires_at?->format('d.m.Y H:i'),
                'isRecurring' => $this->addon->is_recurring,
                'addonsUrl' => url('/app/account/addons'),
            ],
        );
    }
}
