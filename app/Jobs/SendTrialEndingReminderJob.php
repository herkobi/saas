...
use App\Notifications\App\Account\TrialEndingNotification;
...
class SendTrialEndingReminderJob implements ShouldQueue
{
    ...
    {
        $daysList = config('herkobi.subscription.trial_reminder_days');

        if (!is_array($daysList) || $daysList === []) {
            return;
        }

        $daysList = array_values(array_unique(array_filter(
            $daysList,
            fn ($day) => is_int($day) && $day > 0
        )));
        ...
        foreach ($daysList as $days) {
            ...
            Subscription::query()
                ...
                ->each(function (Subscription $subscription) use ($days): void {
                    ...
                    $owner->notify(
                        new TrialEndingNotification($subscription, $days)
                    );
                });
        }
    }
