export function useUserStatus() {
    ...
    const statusIcon = (status: number | undefined): string => {
        const s = status ?? 1;
        if (s === 0) return 'pi pi-ban';
        if (s === 2) return 'pi pi-eye';
        return 'pi pi-check-circle';
    };

    const statusColor = (status: number | undefined): { bg: string; text: string } => {
        const s = status ?? 1;
        if (s === 0) return { bg: 'bg-surface-100 dark:bg-surface-800', text: 'text-surface-500 dark:text-surface-400' };
    ...
    const statusDotColor = (status: number | undefined): string => {
        const s = status ?? 1;
        if (s === 0) return 'bg-surface-400 dark:bg-surface-500';
