...
const userMenuItems = ref([
    {
        label: 'Profil',
        items: [
            { label: 'Profil Düzenle', icon: 'pi pi-user', url: '/panel/profile' },
            { label: 'Şifre Değiştir', icon: 'pi pi-lock', url: '/panel/profile/password' },
            { label: 'Bildirimler', icon: 'pi pi-bell', url: '/panel/profile/notifications' },
        ],
    },
    {
        separator: true,
...
const mainNav: NavItem[] = [
    { title: 'Panel', href: '/panel/dashboard', icon: 'pi pi-home' },
];

const profileNav: NavItem[] = [
    { title: 'Profil', href: '/panel/profile', icon: 'pi pi-user' },
    { title: 'Şifre', href: '/panel/profile/password', icon: 'pi pi-lock' },
    { title: 'İki Adımlı Doğrulama', href: '/panel/profile/two-factor', icon: 'pi pi-shield' },
    { title: 'Bildirimler', href: '/panel/profile/notifications', icon: 'pi pi-bell' },
];

const managementNav: NavItem[] = [
    { title: 'Planlar', href: '/panel/plans', icon: 'pi pi-list' },
    { title: 'Hesaplar', href: '/panel/tenants', icon: 'pi pi-building' },
    { title: 'Abonelikler', href: '/panel/subscriptions', icon: 'pi pi-credit-card' },
    { title: 'Ödemeler', href: '/panel/payments', icon: 'pi pi-wallet' },
    { title: 'Ayarlar', href: '/panel/settings/general', icon: 'pi pi-cog' },
];

const isActive = (href: string) => {
    if (href === '/panel/dashboard') return isCurrentUrl(href);
        ...
        >
            ...
            <div class="flex h-16 items-center gap-3 border-b border-surface-200 px-5 dark:border-surface-800">
                <div class="flex flex-col truncate">
                    ...
                    <span class="truncate text-xs text-surface-500 dark:text-surface-400">Yönetim Paneli</span>
                </div>
            </div>
            ...
            <nav class="flex-1 overflow-y-auto p-4">
                ...
                <!-- Yönetim -->
                <div class="mt-6">
                    <p class="mb-2 px-3 text-xs font-semibold uppercase tracking-wider text-surface-400 dark:text-surface-500">Yönetim</p>
                    <ul class="space-y-1">
                        <li v-for="item in managementNav" :key="item.href">
                            <Link
                                :href="item.href"
                                :class="[
                                    'flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-colors',
